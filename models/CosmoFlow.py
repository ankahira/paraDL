...
class ParallelCosmoFlowSN(Chain):
    def __init__(self):
        super().__init__()
        with self.init_scope():
            self.Conv1 = L.Convolution3D(in_channels=4, out_channels=6, ksize=3, stride=1).to_gpu(0)

            self.Conv2 = L.Convolution3D(in_channels=6, out_channels=32, ksize=4, stride=1).to_gpu(1)
