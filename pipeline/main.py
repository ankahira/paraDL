...
from models import resnet101, resnet152, vgg
...
class Experiments:
    ...
    def gpipe256(model: nn.Module, devices: List[int]) -> Stuffs:
        batch_size = 256
        chunks = 4
        # balance =  [100, 127, 180, 150]  # 4 GPUs
        balance = [19, 20]   # 2 GPUs
...
def cli(ctx: click.Context,
    ...
    #model: nn.Module = resnet152(num_classes=1000, inplace=relu_inplace)

    ## Replace resnet with vgg
    ...
    def run_epoch(epoch: int) -> Tuple[float, float]:
        ...
        for i, (input, target) in enumerate(train_dataloader):
            ...
            torch.cuda.synchronize(in_device)
            stop = time.time()
            with open('weight_update.txt', 'a') as f:
                print(stop-start, file=f)
            scheduler.step()
