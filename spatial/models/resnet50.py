...
class BottleNeckA(chainer.Chain):
    ...
    def __init__(self, comm, in_size, ch, out_size, stride=2):
        ...
        with self.init_scope():
            self.conv1 = LX.Convolution2D(self.comm, 34,
                in_size, ch, 1, stride, 0, initialW=initialW, nobias=True)
            self.bn1 = L.BatchNormalization(ch)
            self.conv2 = LX.Convolution2D(self.comm, 54,
                ch, ch, 3, 1, 1, initialW=initialW, nobias=True)
            self.bn2 = L.BatchNormalization(ch)
            self.conv3 = LX.Convolution2D(self.comm, 43,
                ch, out_size, 1, 1, 0, initialW=initialW, nobias=True)
            self.bn3 = L.BatchNormalization(out_size)

            self.conv4 = LX.Convolution2D(self.comm, 34,
                in_size, out_size, 1, stride, 0, initialW=initialW, nobias=True)
            self.bn4 = L.BatchNormalization(out_size)

    def __call__(self, x):
        h1 = FX.halo_exchange(self.comm, x, k_size=1, index=1, pad=0)
        h1 = F.relu(self.bn1(self.conv1(h1)))
        # revisit this step and fix size k size of halo exchange
        h1 = FX.halo_exchange(self.comm, h1, k_size=1, index=2, pad=1)
        h1 = F.relu(self.bn2(self.conv2(h1)))
        h1 = FX.halo_exchange(self.comm, h1, k_size=1, index=3, pad=0)
        h1 = self.bn3(self.conv3(h1))

        h2 = FX.halo_exchange(self.comm, x, k_size=1, index=4, pad=0)
...
class BottleNeckB(chainer.Chain):
    ...
    def __init__(self, comm, in_size, ch):
        ...
        with self.init_scope():
            self.conv1 = LX.Convolution2D(self.comm, 65,
                in_size, ch, 1, 1, 0, initialW=initialW, nobias=True)
            self.bn1 = L.BatchNormalization(ch)
            self.conv2 = LX.Convolution2D(self.comm, 66,
                ch, ch, 3, 1, 1, initialW=initialW, nobias=True)
            self.bn2 = L.BatchNormalization(ch)
            self.conv3 = LX.Convolution2D(self.comm, 67,
                ch, in_size, 1, 1, 0, initialW=initialW, nobias=True)
            self.bn3 = L.BatchNormalization(in_size)

    def __call__(self, x):
        h = FX.halo_exchange(self.comm, x, k_size=1, index=5, pad=0)
        h = F.relu(self.bn1(self.conv1(h)))
        h = FX.halo_exchange(self.comm, h, k_size=1, index=6, pad=1)
        h = F.relu(self.bn2(self.conv2(h)))
        h = FX.halo_exchange(self.comm, h, k_size=1, index=7, pad=0)
...
class ResNet50(chainer.Chain):
    def __init__(self, comm):
        ...
        self.comm = comm
        with self.init_scope():
            self.conv1 = LX.Convolution2D(self.comm, 1,
    ...
    def __call__(self, x):
        ...
        h = FX.halo_exchange(self.comm, x, k_size=7, index=9, pad=3)
